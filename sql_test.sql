...
CREATE TABLE data_table (column_1 int, column_2 int, column_3 int, join_id int);
...
-- Column_1 needs to be divisible by 2 without creating a decimal number (or modulo) -- Use the bitwise & operator to check for the event numbers in column 1
-- column_2 needs to be smaller than column_1
-- Column_3 needs to end with a 1

SELECT t1.*, t2.email
FROM data_table t1
JOIN email_table t2 ON t1.join_id = t2.join_id
WHERE MOD(column_1, 2) = 0 AND (column_2 < column_1) AND (column_3 LIKE '%1');
